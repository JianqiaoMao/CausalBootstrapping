...
class IDFixSeqTree:
    ...
    Y_star (Set of Strings)     -- set of nodes which are causally before any of the interventional variables: Y* = an_G(V)_{V\A}(Y)  
    ...
    class IDNode:
        """
        A IDNode object to store the intermediate variables in the recursive tree growing and search process.
    ...
    def __init__(self, G, Y_star, X = {}, Y = {}):
        self.G = G
        self.X = X
        self.Y = Y
        self.V = G.nodes()
    ...
    def expand_tree_in_D(self, cur_node):
        """
        A recursive function for tree growing to search all possible fixing orders within a given district D.
        
        Parameters:
        cur_node (IDNode): The current search node.
        ...
        if len(nodes_fix) > 0:
            ...
            if self.node_valid:
                ...
                for fix_node_index in range(len(fixable_nodes)):             
                    ...
                    child_node = self.IDNode(G_fixed = G_fixed_cp, nodes_fix = nodes_fix_cp, 
    ...
    def get_leaf(self, tree):
        """
        A recursive function to retrieve leaf nodes in a tree structure.

        Parameter:
        tree (IDNode): The tree to search the leaf modes.
    ...
    def searchdfixseq(self, D):
        ...
        nodes_fix = VdD.copy()
        root_node = self.IDNode(G_fixed = G_fixed, nodes_fix = nodes_fix, fix_node_seq = [], fix_graph_seq = [])
